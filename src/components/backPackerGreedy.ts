function backPackerGreedy(weights: number[], values: number[], capacity: number): number {
  // محاسبه نسبت ارزش به وزن برای هر کالا
  const n = weights.length;
  const ratios: { ratio: number; value: number; weight: number }[] = [];
...
export default backPackerGreedy;
